...
private fun getNumberOfPathsDfs(input: List<String>) =
    advanceCaveExploration("start", mutableMapOf(), mapOutTheCaveSystem(input), 0)

private fun advanceCaveExploration(
    caveName: String,
    smallCaves: MutableMap<String, Int>,
    ...
    val branchedPathCount = if (cave.name == "end") {
        ...
        cave.tunnels.asSequence().sorted()
            .map { caveMap[it] }.filterNot { it!!.name == "start" }.filterNotNull().sumOf { tunnel ->
                ...
                when {
                    headingToSmallCave && headingToAlreadyVisitedCave -> 0
                    currentlyInSmallCave && headingToClosedCave && notHeadingToEnd -> 0
                    else -> {
                        val updatedPath = if (path == "") {
                            caveName
                        } else {
                            "$path,$caveName"
                        }
...
fun mapOutTheCaveSystem(input: List<String>): Map<String, Cave> {
    val caves = mutableMapOf<String, Cave>()

    input.forEach { tunnel ->
        val from = tunnel.split("-")[0]
        val to = tunnel.split("-")[1]

        if (!caves.containsKey(from)) {
            caves[from] = parseCaveFromName(from)
        }

        if (!caves.containsKey(to)) {
            caves[to] = parseCaveFromName(to)
        }

        caves[from]!!.tunnels.add(to)
        caves[to]!!.tunnels.add(from)
    }

    return caves
}

fun parseCaveFromName(name: String) = Cave(name, name.isSmallCaveName(), mutableSetOf())

fun String.isSmallCaveName() = "[a-z]*".toRegex().matches(this)

data class Cave(val name: String, val isSmall: Boolean, val tunnels: MutableSet<String>)
